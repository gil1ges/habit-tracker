...
def _handle_chart_error(chart_name: str) -> str:
    ...
    return _build_empty_chart("Chart unavailable", "Please try again later.")
...
def build_bar_chart(user) -> str:
    try:
        ...
        habits = stats["habits"]
        if not habits:
            return _build_empty_chart("Completions by Habit", "No habits available yet.")
        ...
        ax.set_title("Completions by Habit")
        ax.set_xlabel("Habit")
...
def build_pie_chart(user) -> str:
    try:
        ...
        if sum(values) == 0:
            return _build_empty_chart("Completed vs Missed", "No completions available yet.")
        ...
        ax.set_title("Completed vs Missed")
        return _fig_to_base64(fig)
    except Exception:
...
def build_line_chart(user) -> str:
    try:
        ...
        if not completion_data:
            return _build_empty_chart("Daily Completions", "No completion history yet.")
        ...
        ax.set_title("Daily Completions")
        ax.set_xlabel("Date")
        ax.set_ylabel("Completions")
...
def build_histogram(user) -> str:
    try:
        ...
        ax.set_title("Completion Distribution by Habit")
        ax.set_xlabel("Completion Count")
        ax.set_ylabel("Number of Habits")
        return _fig_to_base64(fig)
    except Exception:
...
def build_scatter_chart(user) -> str:
    try:
        ...
        habits = stats["habits"]
        if not habits:
            return _build_empty_chart("Target vs Completion", "No habits available yet.")
        ...
        ax.set_title("Target Count vs Completion Count")
